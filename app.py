    ...
    if st.button("確認コードを送る"):
        ...
        if not name or not mail or not pw:
            ...
            if not ok:
                ...
                st.session_state.pending = {"name": name, "email": norm_mail(mail), "password": hash_password(pw), "icon": uploaded_to_uri(icon_up) if icon_up else random.choice(ANIMALS), "code": code, "expires": time.time() + 600, "tries": 0}
