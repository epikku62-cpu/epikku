...
# 展開すると数百MBになる巨大な画像(小さなファイルでも作れる)で、サーバーのメモリが尽きて再起動するのを防ぐ。
# 2500万画素(約5000×5000)を超える画像は読み込まずに断る。
...
def mark_paid_session(session_id):
    ...
    def _update(paid):
        ...
        result["new"] = True
        return paid
    update_json_file(PAID_FILE, {}, _update, backup=True)
    return result["new"]
...
def _period_value(sub, key):
    """current_period_start/end を取得する。
    Stripe API 2025-03-31.basil 以降は Subscription 直下から subscription item 側へ移動したため、
    直下に無ければ items.data[0] から読む。
    (sub.items は dict.items と名前が衝突するので sget 経由で取る)
    """
    v = _subscription_value(sub, key, None)
    if v:
        return v
    data = sget(sget(sub, "items", {}), "data", []) or []
    return sget(data[0], key, None) if data else None

def _latest_invoice_paid(sub):
    """現在の請求書が実際にpaidかを確認する。
    True=支払い確認済み / False=未払い / None=確認不能（一時障害等）
    """
...
def sync_subscription(force=False):
    ...
    invoice_paid = _latest_invoice_paid(sub)
    if invoice_paid is None:
        # Stripeへの一時的な通信失敗でVIPを消さない。
        # 更新ポイントも、支払い確認が取れない間は付与しない。
        return
    if not invoice_paid:
        _clear_vip_state()
        return

    end_ts = _period_value(sub, "current_period_end")
    start_ts = _period_value(sub, "current_period_start")
    try:
        if end_ts:
    ...
    # 同じ更新期間の1200ポイントを、複数タブから二重付与しないように
    # 最新の保存済みstripe_periodをロック内で確認してから付与する。
    def _renew(record):
        old_period = str(record.get("stripe_period") or "")
...
def cancel_subscription_now():
    ...
    save_user_state({"premium_until", "stripe_sub", "stripe_customer"})
...
def credit_pending_checkouts(force=False):
    ...
    mail = norm_mail(st.session_state.get("email"))
    if not name:
        return
    try:
        try:
            # 完了済みだけに絞る(Payment Linkは開くたびに未完了セッションができ、40件の窓がすぐ埋まるため)
            listed = stripe.checkout.Session.list(limit=100, status="complete")
        except Exception:
    ...
    if not isinstance(paid, dict):
        paid = {}
    cur_sub = str(st.session_state.get("stripe_sub") or "").strip()
    for ses in rows:
        meta = sget(ses, "metadata") or {}
        if not isinstance(meta, dict):
            meta = {}
        ref = str(sget(ses, "client_reference_id") or "").strip()
        user = str(meta.get("user") or "").strip()
        owners = [x for x in (ref, user) if x]
        if owners:
            # 決済を作ったときに記録したユーザー名で照合する
            if name not in owners:
                continue
        else:
            # 所有者情報が無い古い決済だけ、決済ページで入力されたメールで照合する
            details = sget(ses, "customer_details") or {}
            det_mail = norm_mail(sget(details, "email"))
            if not det_mail or det_mail != mail:
                continue
        ...
        if not sid:
            continue
        is_sub = str(sget(ses, "mode") or "") == "subscription" or str(meta.get("kind") or "") == "plan"
        if is_sub:
            if cur_sub and str(sget(ses, "subscription") or "") == cur_sub:
                continue          # すでにこのアカウントに紐づけ済み
        elif sid in paid:
            continue              # ポイント購入は反映済み
        apply_checkout_session(sid)

def apply_checkout_session(session_id):
    if not session_id or stripe is None:
        return "決済を確認できません"
    cur = str(st.session_state.get("username") or "").strip()
    if not cur:
        return "ログインしてから操作してください"
    try:
    ...
    # この決済が「いまログインしているアカウントのもの」か確認する。
    # (他人のsession_idを?session_id=で渡して、ポイントやVIPを横取り・使い回しされるのを防ぐ)
    owner = str(meta.get("user") or sget(ses, "client_reference_id") or "").strip()
    if owner:
        if owner != cur:
            return "この決済は別のアカウントのものです"
    else:
        det_mail = norm_mail(sget(sget(ses, "customer_details") or {}, "email"))
        if not det_mail or det_mail != norm_mail(st.session_state.get("email")):
            return "この決済は別のアカウントのものです"
    ...
    if mode == "subscription" or str(meta.get("kind") or "") == "plan":
        ...
        customer = str(sget(ses, "customer") or "")
        bound = {"conflict": False}
        def _bind(users):
            if not isinstance(users, dict):
                return users
            # 1つのサブスクは1アカウントにしか紐づけない(複数アカウントでのVIP・ポイント使い回し防止)
            for n, u in users.items():
                if n != cur and isinstance(u, dict) and str(u.get("stripe_sub") or "") == sub_id:
                    bound["conflict"] = True
                    return users
            if isinstance(users.get(cur), dict):
                users[cur]["stripe_sub"] = sub_id
                users[cur]["stripe_customer"] = customer
            return users
        update_json_file(USERS_FILE, {}, _bind, backup=True)
        if bound["conflict"]:
            return "この月額契約はすでに別のアカウントで使われています"
        st.session_state.stripe_sub = sub_id
        st.session_state.stripe_customer = customer
...
def mark_visit():
    ...
    if who:
        touch_user_seen(who, visit=True)
...
def touch_user_seen(name, visit=False):
    """最終訪問時刻を更新する。visit=True(新しい訪問)のときは累計訪問回数も増やす。
    それ以外は5分に1回まで(操作のたびにusers_data.jsonを書き換えない)。"""
    last = float(st.session_state.get("_seen_at") or 0)
    if not visit and time.time() - last < 300:
        return
    st.session_state._seen_at = time.time()
    def _update(users):
        if not isinstance(users, dict):
            return users
        if name in users and isinstance(users[name], dict):
            ...
            if visit:
                users[name]["visit_count"] = int(users[name].get("visit_count") or 0) + 1
        return users
    update_json_file(USERS_FILE, {}, _update, backup=True)


def record_login(name):
    """ログイン(ID・パスワードでの入室/新規登録)を記録する。Cookieでの自動復帰は「訪問」として別に数える。"""
    now = datetime.now()
    stamp = now.strftime("%Y/%m/%d %H:%M")
    day = now.strftime("%Y/%m/%d")
    def _user(users):
        if isinstance(users, dict) and isinstance(users.get(name), dict):
            users[name]["last_login"] = stamp
            users[name]["last_seen"] = stamp
            users[name]["login_count"] = int(users[name].get("login_count") or 0) + 1
        return users
    def _stats(data):
        if not isinstance(data, dict):
            data = {"total": 0, "days": {}, "last": ""}
        ld = data.get("login_days") if isinstance(data.get("login_days"), dict) else {}
        ld[day] = int(ld.get(day, 0)) + 1
        data["login_days"] = dict(sorted(ld.items())[-60:])
        return data
    try:
        update_json_file(USERS_FILE, {}, _user, backup=True)
        update_json_file(STATS_FILE, {"total": 0, "days": {}, "last": ""}, _stats, backup=True)
        st.session_state._seen_at = time.time()
    except Exception as e:
        print(f"[warn] record_login failed: {e}")
...
def _set_auth_cookie(token="", clear=False):
    secure = "; Secure" if str(SITE_URL).startswith("https://") else ""
    if clear or not token:
        script = f'<script>document.cookie="{AUTH_COOKIE}=; Max-Age=0; Path=/; SameSite=Lax{secure}";</script>'
    else:
        script = f'<script>document.cookie="{AUTH_COOKIE}={token}; Max-Age=2592000; Path=/; SameSite=Lax{secure}";</script>'
    st.markdown(script, unsafe_allow_html=True)
    try:
        import streamlit.components.v1 as components
        components.html(script, height=0, scrolling=False)
...
def _startup_report():
    ...
    try:   # 30日以上前の縮小画像を掃除(必要になれば自動で作り直される)
        cutoff = time.time() - 30 * 86400
        for fname in os.listdir(THUMB_DIR):
            fp = os.path.join(THUMB_DIR, fname)
...
def save_json(path, data, backup=True):
    ...
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as f:
            ...
            f.flush()
            os.fsync(f.fileno())
        # .bak は「今のファイルが正常に読める時だけ」更新する。
        # 壊れたファイルで正常なバックアップを上書きしない。
        if backup and os.path.exists(path) and _read_json_file(path) is not None:
            try:
...
def update_json_file(path, default, updater, backup=True):
    """Read-modify-writeを1回のファイルロック内で行う。
    複数のStreamlitセッションが同時に同じJSONを更新しても、
    後から保存したセッションが先の更新を丸ごと消さないようにする。

    ・本体が読めない場合は .bak から復旧して続行する(load_json と同じ)
    ・本体も .bak も読めず、ファイルが存在する場合は例外にして上書きしない
      (default={} で続行して全データを消すことを防ぐ)
    ・.bak は本体が正常に読めた時だけ更新する
    """
    os.makedirs(os.path.dirname(path) or ".", exist_ok=True)
    lock_file = None
    try:
        if fcntl is not None:
            lock_file = open(path + ".lock", "a+", encoding="utf-8")
            fcntl.flock(lock_file.fileno(), fcntl.LOCK_EX)

        data = _read_json_file(path)
        main_ok = data is not None
        if not main_ok:
            data = _read_json_file(path + ".bak")
            if data is None:
                if os.path.exists(path) and os.path.getsize(path) > 2:
                    raise RuntimeError(f"{os.path.basename(path)} を読めないため、上書きを中止しました。.bak を確認してください")
                data = default
        result = updater(data)
        ...
        try:
            with os.fdopen(fd, "w", encoding="utf-8") as f:
                ...
                f.flush()
                os.fsync(f.fileno())
            if backup and main_ok and os.path.exists(path) and os.path.getsize(path) > 2:
                try:
                    shutil.copy2(path, path + ".bak")
                except Exception:
                    pass
...
def name_taken(users, name):
    """ユーザー名の重複チェック。大文字小文字・前後の空白の違いも同じ名前として扱う。"""
    nl = str(name or "").strip().casefold()
    return any(str(k).strip().casefold() == nl for k in users)

# ---- ログイン試行の回数制限(サーバー全体で共有・メモリ上) ----
LOGIN_MAX_FAILS = 5
LOGIN_LOCK_SEC = 600

@st.cache_resource
def _login_fail_store():
...
def login_blocked(key):
    ...
    with s["lock"]:
        ...
        if not rec:
            return False
        if time.time() - rec["first"] > LOGIN_LOCK_SEC:
...
def login_failed(key):
    ...
    with s["lock"]:
        if len(s["fails"]) > 5000:
            s["fails"].clear()
        rec = s["fails"].get(key)
        if not rec or now - rec["first"] > LOGIN_LOCK_SEC:
            rec = {"n": 0, "first": now}
        rec["n"] += 1
        s["fails"][key] = rec
...
def find_user(users, key):
    ...
    if not key:
        return None
    k = norm_mail(key)
    # メールアドレスの形なら、ユーザー名より先にメールで探す
    # (他人のメールと同じ名前で登録されてもログインできなくならないように)
    if "@" in key:
        for name, v in users.items():
            if isinstance(v, dict) and norm_mail(v.get("email")) == k:
                return name
    if key in users:
        return key
    for name, v in users.items():
...
def uploaded_to_uri(uploaded, max_mb=None):
    """アップロードされた画像を data URI にする。大きすぎるファイルは断る(メモリ節約)。"""
    limit = (max_mb or UPLOAD_IMAGE_MAX_MB) * 1024 * 1024
    if int(getattr(uploaded, "size", 0) or 0) > limit:
        raise Exception(f"画像は{max_mb or UPLOAD_IMAGE_MAX_MB}MB以下にしてください")
...
def migrate_stored_images(record, username=""):
    ...
    if isinstance(icon, str) and icon.startswith("data:"):
        try:
            ...
            print(f"[warn] icon migrate failed: {e}")
            path = ""
            record["icon"] = random.choice(ANIMALS)      # 読み込めないアイコンは動物アイコンに戻す
            changed = True
        if path:
            record["icon"] = path
            changed = True
    ...
    for item in lib:
        ...
        if isinstance(url, str) and url.startswith("data:"):
            path = _store_quiet(url, LIBRARY_DIR, f"{prefix}_lib")
            if path:
                item = dict(item)
                item["url"] = path
                changed = True
    ...
    for item in hist:
        ...
        url = item.get("url") or ""
        if isinstance(url, str) and url.startswith("data:"):
            path = _store_quiet(url, LIBRARY_DIR, f"{prefix}_hist")
            if path:
                item = dict(item)
                item["url"] = path
                changed = True
...
def _store_quiet(uri, folder, name_hint="img"):
    """store_image_uri の、失敗しても例外を出さない版(ログイン時の画像移行用)。"""
    try:
        return store_image_uri(uri, folder, name_hint)
    except Exception as e:
        print(f"[warn] image migrate failed: {e}")
        return ""


def _make_small_jpeg(src_path, out_path, max_side, quality):
    """画像を max_side に収まる小さなJPEGにして保存する(JPEGは縮小して読み込むのでメモリも少ない)。"""
    img = Image.open(src_path)
    if getattr(img, "format", "") == "JPEG":
        half = max(1, int(max_side) // 2)
        img.draft("RGB", (half, half))
    img.load()
    if img.mode != "RGB":
        img = img.convert("RGB")
...
def thumb_path(path, size=320, quality=80):
    """一覧などの小さな表示用に、縮小したJPEGをディスクにキャッシュして、そのパスを返す。
    原寸の画像を何枚も読み込んでメモリを使うのを防ぐ。縮小できない場合は元のパスをそのまま返す。"""
    local = _safe_local_path(path)
    if not local:
        return path
    try:
        stt = os.stat(local)
        key = hashlib.sha1(f"{local}|{stt.st_mtime_ns}|{stt.st_size}|{size}|{quality}".encode()).hexdigest()[:24]
        out = os.path.join(THUMB_DIR, f"{key}.jpg")
        if not os.path.exists(out):
            _make_small_jpeg(local, out, size, quality)
        return out
    except Exception:
        return local


def optimized_static_image(path, max_side=1400, quality=75):
    """アプリに同梱した大きな画像(背景・ヘッダー)を、表示に十分な大きさに縮小してキャッシュする。
    すでに小さいもの(200KB未満)は、そのまま使う。"""
    try:
        if not path or not os.path.exists(path):
            return path
        stt = os.stat(path)
        if stt.st_size < 200 * 1024:
            return path
        key = hashlib.sha1(f"{os.path.abspath(path)}|{stt.st_mtime_ns}|{stt.st_size}|{max_side}|{quality}".encode()).hexdigest()[:24]
        out = os.path.join(THUMB_DIR, f"static_{key}.jpg")
        if not os.path.exists(out):
            _make_small_jpeg(path, out, max_side, quality)
        return out
    except Exception:
        return path
...
def release_memory():
    """重い処理のあとに、使い終わったメモリをOSへ返す(Pythonは解放しても、OSへ返さないことがあるため)。"""
    try:
        gc.collect()
        import ctypes
...
def memory_status():
    """このアプリのプロセスのメモリ使用量(MB)。取得できない項目は None。"""
    info = {"rss_mb": None, "peak_mb": None, "limit_mb": None}
    try:
    ...
    try:
        import resource
        info["peak_mb"] = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1024.0   # Linuxでは単位がKB
    except Exception:
        pass
    limit = None
    for p in ("/sys/fs/cgroup/memory.max", "/sys/fs/cgroup/memory/memory.limit_in_bytes"):
        try:
            with open(p, "r") as f:
                v = f.read().strip()
            if v.isdigit() and int(v) < (1 << 45):
                limit = int(v) / 1024.0 / 1024.0
                break
        except Exception:
            continue
    env_limit = os.environ.get("MEMORY_LIMIT_MB", "").strip()
...
def log_memory_maybe():
    """5分に1回、メモリ使用量をサーバーログ(Renderの Logs)に出す。再起動の原因を後から調べられるように。"""
    state = _mem_log_state()
    now = time.time()
    if now - state["t"] < 300:
        return
    state["t"] = now
    m = memory_status()
    if m["rss_mb"] is not None:
        print(f"[mem] 使用中={m['rss_mb']:.0f}MB 起動後の最大={(m['peak_mb'] or 0):.0f}MB 上限={('%.0f' % m['limit_mb']) if m['limit_mb'] else '不明'}MB")


def uri_to_image(uri, max_side=None):
    """画像を RGB で読み込む。
    max_side を指定すると、JPEGは縮小した状態で読み込み(メモリと時間を節約)、後で max_side に収めるときの下ごしらえになる。"""
    if not uri:
        return None
    try:
        if isinstance(uri, str) and uri.startswith("data:"):
            source = BytesIO(base64.b64decode(uri.split(",", 1)[1]))
        else:
            source = _safe_local_path(uri)
            if not source:
                raise Exception("外部URLの画像は読み込めません")
        img = Image.open(source)
        if max_side and getattr(img, "format", "") == "JPEG":
            half = max(1, int(max_side) // 2)
            img.draft("RGB", (half, half))     # 1/2・1/4・1/8に縮小して読み込む(元が大きいほど効果が大きい)
        img.load()
        return img if img.mode == "RGB" else img.convert("RGB")
    except (Image.DecompressionBombError, Image.DecompressionBombWarning):
        raise Exception("画像が大きすぎます。縦×横が2500万画素以内(約5000×5000)の画像にしてください")
...
def _history_within_days(history, days=3):
    """画像生成履歴を直近N日だけ残す。壊れた/日付不明の古い項目は保存しない。"""
...
def save_user_state(fields=None):
    """ユーザー情報を安全に部分更新する。

    Streamlitの古いsession_stateをユーザー情報全体として保存すると、
    別タブ・別セッションで先に保存された最新データを古い状態で
    上書きする危険があるため、指定された項目だけを最新のusers_data.json
    に対してread-modify-writeする。
    """
    ...
    # 認証情報・プロフィール・決済・履歴・保存庫など、明示的に指定された項目だけ保存する。
    allowed = {
        "email", "icon", "characters", "points", "signup_points_remaining",
        "community_seen_at", "premium_until", "stripe_sub", "stripe_customer",
        "stripe_period", "history", "library"
    }
    fields &= allowed
    ...
    for key in fields:
        ...
        if key in ("points", "signup_points_remaining"):
            try:
                value = int(value or 0)
            except Exception:
                value = 0
        elif key == "community_seen_at":
            try:
                value = float(value or 0)
            except Exception:
                value = 0.0
        elif key in ("characters", "history", "library"):
            value = value if isinstance(value, list) else []
        state_values[key] = value

    password = st.session_state.get("password_hash")
    update_last_seen = bool(st.session_state.get("logged_in"))
    ...
    def _update(users):
        ...
        for key, value in state_values.items():
            if key == "history":
                record[key] = _history_within_days(value, 3)
            else:
                record[key] = value

        # 新規項目の初期値。既存値がある場合は絶対に消さない。
        defaults = {
            "email": "", "icon": random.choice(ANIMALS), "characters": [],
            "points": 0, "signup_points_remaining": 0, "community_seen_at": 0.0,
            "premium_until": "", "stripe_sub": "", "stripe_customer": "",
            "stripe_period": "", "history": [], "library": []
        }
        ...
        record["rank"] = "vip" if str(record.get("premium_until") or "") and _is_premium_until(record.get("premium_until")) else "ブロンズ"
        if update_last_seen:
            record["last_seen"] = datetime.now().strftime("%Y/%m/%d %H:%M")
        elif "last_seen" not in record:
            record["last_seen"] = ""
        users[name] = record
        return users
...
def update_user_atomic(updater):
    """最新のユーザーデータをロック内で更新する共通処理。"""
    name = st.session_state.get("username")
    if not name:
        return None
    ...
    def _update(users):
        ...
        prev = users.get(name) if isinstance(users.get(name), dict) else {}
        record = dict(prev)
        result = updater(record)
        if result is not None:
            record = result
        users[name] = record
        result_holder["record"] = dict(record)
        return users
...
def _sync_session_from_user_record(record, keys):
    if not isinstance(record, dict):
        return
    for key in keys:
        if key in record:
...
def _remove_board_image(path):
    try:
        root = os.path.abspath(BOARD_DIR)
        p = os.path.abspath(str(path or ""))
        if path and p.startswith(root + os.sep) and os.path.isfile(p):
            os.remove(p)
    except Exception:
        pass
...
def board_delete_post(post_id, username, owner=False):
    ...
    def mut(current):
        ...
        return current
    update_board(mut)
    _remove_board_image(removed["path"])
...
def board_image_uri(post):
    ...
    if path and os.path.exists(path):
        return path      # ファイルのパスをそのまま渡す(全体をbase64にして、メモリを二重に使わない)
    return post.get("url") or ""

def add_history_item(item):
    """生成履歴を最新データへ原子的に追加し、3日分だけ保持する。"""
    if not isinstance(item, dict):
        return
    ...
    if record is not None:
        st.session_state.simple_history = _history_within_days(record.get("history", []), 3)
...
def add_library(uri, label="", extra=None):
    ...
    def _add(record):
        library = record.get("library") if isinstance(record.get("library"), list) else []
        record["library"] = library + [item]
        return record

    record = update_user_atomic(_add)
    if record is not None:
        st.session_state.library = record.get("library", [])
...
def adjust_user_points(delta, signup_delta=0, reason="", refund=False):
    """最新の保存済みポイントを基準に原子的に増減する。
    refund=True の増加は「付与」ではなく「使用分の取り消し」として集計する。"""
    delta = int(delta)
    signup_delta = int(signup_delta)

    def _update(record):
        current = int(record.get("points") or 0)
        ...
        if delta < 0:
            ...
            if refund:
                record["points_used"] = max(0, int(record.get("points_used") or 0) - delta)
            else:
    ...
    try:
        record = update_user_atomic(_update)
...
class NoRefundError(Exception):
    """外部APIが「受け付けたかもしれない」状態で失敗した場合。二重生成を避けるためポイントは返さない。"""

def take_points(cost, reason="生成"):
    """ポイントを消費する。実際に消費した場合は True、消費しなかった場合(無料・管理者)は False を返す。
    生成に失敗したら、True が返っていた時だけ refund_points() で返却すること。"""
...
def refund_points(cost, reason="生成失敗の返却"):
    """生成に失敗した時にポイントを戻す(失敗しても画面は止めない)。"""
    try:
        cost = int(cost)
        if cost > 0:
            adjust_user_points(cost, reason=reason, refund=True)
    except Exception as e:
        print(f"[warn] refund failed: {e}")

def _refund_job(job):
    """動画ジョブが失敗で終わった時に、開始時に消費したポイントを戻す。"""
    try:
        cost = int((job or {}).get("cost") or 0)
    except Exception:
        cost = 0
    if cost > 0:
        refund_points(cost, reason="動画生成失敗の返却")

def _save_history_safely(item):
    """履歴の保存に失敗しても、生成できた画像は表示する。"""
    try:
...
def _delete_simple_char(i):
    """キャラ削除ボタンのコールバック。入力欄(ウィジェット)の値も詰め直す。"""
    chars = list(st.session_state.get("schars") or [])
    bubs = list(st.session_state.get("sbubbles") or [])
    if 0 < i < len(chars):
        chars.pop(i)
        if i < len(bubs):
            bubs.pop(i)
    bubs = (bubs + [""] * len(chars))[:len(chars)]
    st.session_state.schars = chars
    st.session_state.sbubbles = bubs
    for j in range(3):
        if j < len(chars):
            st.session_state[f"scarea_{j}"] = chars[j]
            st.session_state[f"sbb_{j}"] = bubs[j]
        else:
            st.session_state.pop(f"scarea_{j}", None)
            st.session_state.pop(f"sbb_{j}", None)

def logout_reset():
    """ログアウト時に、前のユーザーの情報をセッションから消す。"""
    for k, v in {
        "logged_in": False, "username": "", "email": "", "password_hash": "", "points": 0,
        "premium_until": "", "stripe_sub": "", "stripe_customer": "", "stripe_period": "",
    ...
    for k in ("signup_points_remaining", "vmove_vid", "vmove_img", "vmove_out",
              "_stripe_sync_at", "_stripe_checkout_at", "_seen_at"):
...
def nai_request(prompt, width, height, model, steps=23, scale=5.0, negative="", char_texts=None, char_refs=None, style_refs=None, sampler="k_euler_ancestral", seed=None, set_refs=None):
    if not NAI_KEY:
    ...
    if not is_v5 and (model.startswith("nai-diffusion-4-5") or model.startswith("nai-diffusion-4")):
        refs, kinds, strengths, seconds = [], [], [], []
        # セット(精密画像参照): 1枚ごとに タイプ・強さ・忠実度 を指定して送る
        for r in (set_refs or [])[:SET_MAX_REFS]:
            if not isinstance(r, dict) or not r.get("uri"):
                continue
            refs.append(pad_ref(r["uri"]))
            kinds.append(r.get("kind") if r.get("kind") in SET_KIND_VALUES else "character&style")
            strengths.append(_unit(r.get("strength"), 1.0))
            seconds.append(_unit(r.get("secondary"), 0.75))
        if not refs:
            # 従来の指定方法(char_refs / style_refs): 1枚だけ使う
...
def mm_create_video(payload):
    ...
    if res.status_code not in (200, 201, 202):
        ...
        if res.status_code == 500:
            raise NoRefundError("MiniMax側で一時的なエラーが発生しました。少し待ってから、もう一度お試しください。")
        raise Exception(f"動画生成APIエラー {res.status_code}: {res.text[:300]}")
    try:
        data = res.json()
    except Exception:
        raise NoRefundError("動画生成APIから正しい応答を受け取れませんでした。少し待ってから再試行してください。")
...
def show_header():
    if os.path.exists(HEADER_IMG):
        st.image(optimized_static_image(HEADER_IMG, 1400, 78), use_container_width=True)
...
def apply_login(name, data, persist=True, sync=True, pending=True):
    ...
    # ログイン時は読み込んだユーザーデータをそのまま再保存しない。
    # 古いセッションが最新の保存庫・ポイント・決済情報を上書きするのを防ぐ。
    if sync:
        sync_subscription()
    if pending:
...
def render_top_menu():
    ...
    st.markdown(
        ...
        div.st-key-panel_menu_drawer,
        div.st-key-panel_menu_drawer p,
        div.st-key-panel_menu_drawer h1,
        div.st-key-panel_menu_drawer h2,
        div.st-key-panel_menu_drawer h3,
        div.st-key-panel_menu_drawer span,
        div.st-key-panel_menu_drawer label,
        div.st-key-panel_menu_drawer [data-testid="stMarkdownContainer"],
        div.st-key-panel_menu_drawer [data-testid="stMarkdownContainer"] * {
    ...
    with st.container(key="panel_menu_drawer"):
        ...
        if st.session_state.logged_in:
            ...
            if isinstance(icon, str) and (icon.startswith("data:image") or _safe_local_path(icon)):
                st.image(thumb_path(media_src(icon), 120), width=48)
            else:
                st.write(icon)
            pts = int(st.session_state.get("points") or 0)
            name = html_lib.escape(str(st.session_state.get("username") or ""))
            st.markdown(
                f'<div style="color:#111;font-size:18px;font-weight:800;line-height:1.45;margin:4px 0 10px;">'
                f'{name}<br><span style="color:#ff4d88;">残り {pts}ポイント</span>'
                f'</div>',
                unsafe_allow_html=True,
            )
        ...
        if st.session_state.logged_in:
            st.markdown(
                f'<div style="color:#111;font-weight:700;margin:0 0 10px;">会員 {html_lib.escape(member_label())}</div>',
                unsafe_allow_html=True,
            )
        else:
            st.markdown(
                f'<div style="color:#111;font-weight:700;margin:0 0 10px;">ポイント {int(st.session_state.get("points") or 0)}<br>会員 未登録</div>',
                unsafe_allow_html=True,
            )
...
def fidelity_to_secondary(fidelity):
    """画面の「忠実度」(1に近いほど参照を強く守る)を、NovelAI APIの secondary_strength に変換する。
    忠実度と secondary_strength は逆向き(secondary = 1 - fidelity)という前提で実装している。
    実際に生成して向きが逆だと感じたら、この1行だけ `return round(_unit(fidelity, 0.75), 2)` に変える。"""
    return round(1.0 - _unit(fidelity, 0.75), 2)


def _set_id(entry, i):
    return str((entry or {}).get("id") or f"legacy{i}")


def set_display_name(entry):
    kind = SET_KIND_LABEL.get((entry or {}).get("kind") or "", "")
    return char_label(entry or {}) + (f"({kind})" if kind else "")


def set_refs_from_picks(entries, picked_ids):
    """選ばれたセットから、nai_request に渡す参照画像のリストを作る。"""
    refs = []
    for i, e in enumerate(entries or []):
        if not isinstance(e, dict) or _set_id(e, i) not in (picked_ids or []):
            continue
        if e.get("image"):
            kind = e.get("kind") if e.get("kind") in SET_KIND_VALUES else "character&style"
            refs.append({"uri": e["image"], "kind": kind, "strength": _unit(e.get("strength"), 1.0),
                         "secondary": fidelity_to_secondary(e.get("fidelity", 0.75)), "name": char_label(e)})
            continue
        # 旧形式のセット(char_refs / style_refs)は、従来どおり1枚として使う
        cr, sr = _set_refs_from_entry(e)
        if cr and sr:
            uri, kind = cr[0]["uri"], "character&style"
        elif cr:
            uri, kind = cr[0]["uri"], "character"
        elif sr:
            uri, kind = sr[0]["uri"], "style"
        else:
            continue
        refs.append({"uri": uri, "kind": kind, "strength": 1.0, "secondary": 0.75, "name": char_label(e)})
    return refs[:SET_MAX_REFS]


def add_ref_set(name, uri, kind, strength=1.0, fidelity=0.75):
    """セットを登録する(参照画像はファイルに保存し、ユーザーデータには保存先だけを持つ)。"""
    name = str(name or "").strip()
    user = str(st.session_state.get("username") or "")
    if not st.session_state.get("logged_in") or not user:
        raise Exception("セットを登録するにはログインが必要です")
    if not name:
        raise Exception("セット名を入れてください")
    if len(name) > MAX_NAME_LEN:
        raise Exception(f"セット名は{MAX_NAME_LEN}文字以内にしてください")
    if kind not in SET_KIND_VALUES:
        raise Exception("タイプを選んでください")
    if not uri:
        raise Exception("参照画像を選んでください")
    if len(st.session_state.get("characters") or []) >= MAX_SETS:
        raise Exception(f"セットは{MAX_SETS}個までです。不要なセットを削除してください")
    prefix = re.sub(r"[^a-zA-Z0-9_-]+", "_", user)[:24] or "user"
    try:
        path = store_image_uri(uri, SET_DIR, f"{prefix}_set")
    except Exception:
        raise Exception("画像を読み込めませんでした。別の画像でお試しください")
    if not path:
        raise Exception("画像を保存できませんでした")
    item = {"id": uuid.uuid4().hex[:12], "name": name, "kind": kind, "image": path,
            "strength": round(_unit(strength, 1.0), 2), "fidelity": round(_unit(fidelity, 0.75), 2),
            "time": datetime.now().strftime("%Y/%m/%d %H:%M")}

    def _add(record):
        chars = record.get("characters") if isinstance(record.get("characters"), list) else []
        if len(chars) >= MAX_SETS:
            raise ValueError(f"セットは{MAX_SETS}個までです")
        record["characters"] = list(chars) + [item]
        return record
    ...
    try:
        ...
        raise Exception(str(e))
    if record is not None:
        st.session_state.characters = record.get("characters", [])
    return item


def delete_ref_set(set_id):
    """セットを削除する(SET_DIR内の画像ファイルも一緒に消す)。"""
    removed = {"path": ""}
    ...
    def _del(record):
        ...
        for i, e in enumerate(chars):
            if _set_id(e, i) == set_id:
                removed["path"] = str((e or {}).get("image") or "")
                continue
            keep.append(e)
        record["characters"] = keep
        return record

    record = update_user_atomic(_del)
    if record is not None:
        st.session_state.characters = record.get("characters", [])
    path = _safe_local_path(removed["path"])
    if path and os.path.abspath(path).startswith(os.path.abspath(SET_DIR) + os.sep):
        try:
            os.remove(path)
        except Exception:
            pass
...
def render_set_manager():
    """セットの登録・一覧・削除の画面。"""
    entries = st.session_state.get("characters") or []
    n = int(st.session_state.get("setnew_n") or 0)
    if st.session_state.get("set_notice"):
        st.success(st.session_state.pop("set_notice"))
    with st.expander(f"➕ セットを登録する({len(entries)}/{MAX_SETS})", expanded=not entries):
        st.caption("参照画像の「絵柄」や「キャラクター」を、生成する絵に反映するための登録です。"
                   "タイプ: キャラクター=顔や髪型・服など人物の特徴 / 絵柄=タッチや色使い / 両方。")
        if not st.session_state.get("logged_in"):
            st.info("ログインするとセットを登録できます。")
        else:
            name = st.text_input("セット名", key=f"setnew_name_{n}", max_chars=MAX_NAME_LEN)
            up = st.file_uploader("参照画像", type=["png", "jpg", "jpeg", "webp"], key=f"setnew_file_{n}")
            kind_label = st.radio("タイプ", list(SET_KINDS.keys()), horizontal=True, key=f"setnew_kind_{n}")
            strength = st.slider("強さ", 0.0, 1.0, 1.0, 0.05, key=f"setnew_strength_{n}",
                                 help="高いほど参照画像の特徴が強く出ます。ポーズや表情まで似すぎるときは下げてください。")
            fidelity = st.slider("忠実度", 0.0, 1.0, 0.75, 0.05, key=f"setnew_fidelity_{n}",
                                 help="高いほど参照を忠実に守り、プロンプトで上書きしにくくなります。")
            if up is not None:
                st.image(up, width=160)
            saved = False
            if st.button("この内容で登録", type="primary", key=f"setnew_save_{n}"):
                try:
                    if up is None:
                        raise Exception("参照画像を選んでください")
                    if int(getattr(up, "size", 0) or 0) > 10 * 1024 * 1024:
                        raise Exception("画像は10MB以下にしてください")
                    add_ref_set(name, uploaded_to_uri(up), SET_KINDS[kind_label], strength, fidelity)
                    st.session_state.set_notice = f"セット「{str(name).strip()}」を登録しました"
                    st.session_state.setnew_n = n + 1      # 入力欄を空に戻す
                    saved = True
                except Exception as e:
                    st.error(str(e))
            if saved:
    ...
    if entries:
        ...
        for i, e in enumerate(entries):
            ...
            with c_img:
                path = _safe_local_path(e.get("image") or "")
                if path:
                    st.image(path, width=80)
            with c_info:
                st.write(char_label(e))
                if e.get("image"):
                    st.caption(f"{SET_KIND_LABEL.get(e.get('kind') or '', '-')}　強さ {e.get('strength', 1.0)}　忠実度 {e.get('fidelity', 0.75)}")
                else:
                    st.caption("旧形式のセット")
            with c_del:
                if st.button("削除", key=f"setdel_{sid}"):
                    delete_ref_set(sid)
                    st.rerun()


def _set_refs_from_entry(entry):
    """登録済みのセット/キャラクター情報から精密参照用画像を取り出す。"""
    if not isinstance(entry, dict):
        return [], []
    char_keys = ("char_refs", "character_refs", "character_images", "character_reference_images")
    style_keys = ("style_refs", "style_images", "style_reference_images")
    both_keys = ("refs", "reference_images", "images")
    char_refs = []
    style_refs = []
    for k in char_keys:
        if entry.get(k):
            char_refs = normalize_refs(entry.get(k))
            break
    for k in style_keys:
        if entry.get(k):
            style_refs = normalize_refs(entry.get(k))
            break
    if not char_refs and not style_refs:
        for k in both_keys:
            if entry.get(k):
                refs = normalize_refs(entry.get(k))
                if refs:
                    char_refs = refs
                break
...
def render_set_generation_page():
    ...
    st.text_area("キャラクタープロンプト", key="set_sc")
    st.text_area("その他プロンプト", key="set_so")
    st.text_area("除外プロンプト", key="set_sn")

    size_opts = [k for k, v in SIMPLE_SIZES.items() if (is_premium() or is_owner() or not v["paid"])]
    current_size = st.session_state.get("set_size") or (size_opts[0] if size_opts else "")
    ...
    with st.expander("詳細な生成設定", expanded=False):
        ...
        current_sampler = st.session_state.get("set_sampler") or sampler_options[0]
        if current_sampler not in sampler_options:
            current_sampler = sampler_options[0]
            st.session_state.set_sampler = current_sampler
        sampler_name = st.selectbox("サンプラー", sampler_options, index=sampler_options.index(current_sampler), key="set_sampler")
        seed_text = st.text_input("シード値", key="set_seed", placeholder="空欄ならランダム")
        if seed_text.strip():
            try:
    ...
    if st.button("生成する", type="primary", key="set_generate"):
        st.session_state.set_busy = True
        st.rerun()
    if st.session_state.get("set_busy"):
        st.session_state.set_busy = False   # 先に下ろす(途中で中断されても、次に開いた時に自動で再課金・再生成されない)
        charged = False
        try:
            with st.spinner("生成中…"):
                ...
                img = nai_request(
                    ...
                    steps=steps, scale=scale, negative=st.session_state.set_sn.strip(),
                    set_refs=refs, sampler=sampler_labels[sampler_name], seed=used_seed,
                )
            ...
            if charged:
                refund_points(total_cost)
            st.session_state.set_error = str(e)
        release_memory()
        go("chars")
        st.rerun()

    if st.session_state.get("set_error"):
        st.error(st.session_state.set_error)
    if st.session_state.get("set_image"):
        st.image(st.session_state.set_image, use_container_width=True)
        st.caption(f"シード値: {st.session_state.get('set_last_seed')}")
        raw = uri_to_image(st.session_state.set_image)
        st.download_button("PNG保存", data=image_to_bytes(raw), file_name="set.png", mime="image/png", key="set_download")
        if st.button("この画像を動画にする", key="set_to_video"):
...
restore_login()
mark_visit()
if st.session_state.logged_in and stripe is not None and st.session_state.get("stripe_sub"):
    try:
        _until = datetime.fromisoformat(str(st.session_state.get("premium_until") or ""))
        _need_sync = _until - datetime.now() < timedelta(days=2)
    except Exception:
        _need_sync = True
    if _need_sync:
...
if st.session_state.page == "home":
    b64 = file_b64(optimized_static_image(HOME_IMG, 1800, 72))
    if b64:
    ...
    panel AIは<br>画像生成・動画生成<br>作成AIサイト ♡
    </div>
    """, unsafe_allow_html=True)
...
if st.session_state.error:
    ...
    for i, item in enumerate(list(reversed(st.session_state.library))[:lib_limit]):
        ...
        with cols[-1]:
            if st.button("消す", key=f"libd_{i}"):
                ...
                item_url = str(target.get("url") or "")
                def _delete(record):
                    lib = record.get("library") if isinstance(record.get("library"), list) else []
                    keep = []
                    removed = False
                    for x in lib:
                        xid = str((x or {}).get("id") or "")
                        if item_id:
                            hit = (xid == item_id)
                        else:   # idの無い古い項目は、同じURLの最初の1件だけを消す
                            hit = (not removed) and str((x or {}).get("url") or "") == item_url
                        if hit:
                            removed = True
                            continue
                        keep.append(x)
                    record["library"] = keep
                    # 他の保存庫項目・履歴が同じファイルを使っていなければ、ファイルも消す
                    path = _safe_local_path(item_url)
                    if path:
                        hist = record.get("history") if isinstance(record.get("history"), list) else []
                        used = any(str((y or {}).get("url") or "") == item_url for y in keep) or \
                               any(isinstance(h, dict) and str(h.get("url") or "") == item_url for h in hist)
                        if not used:
                            try:
                                os.remove(path)
                            except Exception:
                ...
                if record is not None:
                    st.session_state.library = record.get("library", [])
                st.rerun()
    if len(st.session_state.library) > lib_limit:
        st.button(f"さらに表示({len(st.session_state.library) - lib_limit}件)", key="lib_more", on_click=_lib_more, use_container_width=True)
    release_memory()
    ...
    if st.button("動画にする", type="primary"):
        if st.session_state.get("vjob") or st.session_state.get("video_starting"):
            ...
            charged = False
            try:
                charged = take_points(video_cost(dur), reason="動画生成")
                task_id = grok_start_video(st.session_state.video_src, motion, dur)
                st.session_state.vjob = {"kind": "video", "id": task_id, "cost": video_cost(dur) if charged else 0}
                start_wait(); st.session_state.error = ""
            except Exception as e:
                if charged and not isinstance(e, NoRefundError):
                    refund_points(video_cost(dur))
                st.session_state.error = str(e)
            finally:
    ...
    if job and job.get("kind") == "vmove":
        ...
        if st.session_state.get("vjob") or st.session_state.get("video_starting"):
            ...
            charged = False
            try:
                charged = take_points(cost, reason="動画を移す")
                task_id = mm_start_move(st.session_state.vmove_img, st.session_state.vmove_vid, motion, dur)
                st.session_state.vjob = {"kind": "vmove", "id": task_id, "cost": cost if charged else 0}
                start_wait(); st.session_state.error = ""
            except Exception as e:
                if charged and not isinstance(e, NoRefundError):
                    refund_points(cost)
    ...
    if st.button("この画像にする", type="primary") and up:
        ...
        icon_saved = False
        try:
            st.session_state.icon = store_image_uri(uploaded_to_uri(up, max_mb=3), ICON_DIR, f"{prefix}_icon") or random.choice(ANIMALS)
            save_user_state({"icon"})
            icon_saved = True
        except Exception as e:
            st.error(str(e))
        if icon_saved:
    ...
    if st.button("確認コードを送る"):
        ...
        if not name or not mail or not pw:
        ...
        elif icon_up is not None and int(getattr(icon_up, "size", 0) or 0) > 3 * 1024 * 1024:
            st.error("アイコン画像は3MB以下にしてください")
            ...
            if not ok:
                print(f"[warn] send_code_mail failed: {err}")
                st.error("メールを送れませんでした。時間をおいてもう一度お試しください")
            else:
                st.session_state._code_sent_at = time.time()
                st.session_state.pending = {"name": name, "email": norm_mail(mail), "password": hash_password(pw), "icon": uploaded_to_uri(icon_up, max_mb=3) if icon_up else random.choice(ANIMALS), "code": code, "expires": time.time() + 600, "tries": 0}
    ...
    if st.session_state.pending:
        ...
        if st.button("登録する", type="primary"):
            ...
            users = load_json(USERS_FILE, {})
            if time.time() > float(p.get("expires") or 0):
                st.error("確認コードの有効期限(10分)が切れました。もう一度コードを送ってください")
                st.session_state.pending = None
            elif int(p.get("tries") or 0) >= 5:
                st.error("入力の失敗が多すぎます。もう一度コードを送ってください")
                st.session_state.pending = None
            elif not hmac.compare_digest(code_in.strip().encode("utf-8"), str(p["code"]).encode("utf-8")):
                ...
                def _create_user(current):
                    if not isinstance(current, dict):
                        current = {}
                    if email_taken(current, p["email"]) or name_taken(current, p["name"]):
                        return current
                    current[p["name"]] = new_user
                    created["ok"] = True
                    created["user"] = dict(new_user)
                    return current
                update_json_file(USERS_FILE, {}, _create_user, backup=True)
                if not created["ok"]:
                    st.error("すでに登録されています")
                else:
    ...
    if st.button("ログインする"):
        lkey = (lu or "").strip().casefold()
        if login_blocked(lkey):
            st.error("ログインの失敗が続いたため、しばらく時間をおいてからお試しください")
        else:
            users = load_json(USERS_FILE, {})
    ...
    if not st.session_state.logged_in:
        ...
        if st.button("ログイン / 登録", type="primary", use_container_width=True):
            go("register"); st.rerun()
    elif stripe is None or not STRIPE_SECRET_KEY:
        st.error("決済設定がまだです。")
    else:
        ...
        if is_premium():
            ...
            if until:
                try:
                    st.write(f"VIP有効期限: {datetime.fromisoformat(until).strftime('%Y/%m/%d %H:%M')}")
                except Exception:
                    pass
            ...
            elif st.button(f"月額 {MONTHLY_PRICE}円で登録", type="primary", use_container_width=True):
                try:
                    session = stripe_checkout(
                        "subscription",
    ...
    viewers = data.get("viewers") if isinstance(data.get("viewers"), dict) else {}
    guest_days = data.get("guest_days") if isinstance(data.get("guest_days"), dict) else {}
    login_days = data.get("login_days") if isinstance(data.get("login_days"), dict) else {}
    ...
    users = load_json(USERS_FILE, {})
    valid_users = [(name, u) for name, u in (users.items() if isinstance(users, dict) else []) if isinstance(u, dict)]

    def _t(s):
        try:
            return datetime.strptime(str(s or ""), "%Y/%m/%d %H:%M")
        except Exception:
            return None

    def _fmt(t, date_only=False):
        return t.strftime("%Y/%m/%d" if date_only else "%Y/%m/%d %H:%M") if t else "-"

    ONLINE_MIN = 30
    user_rows = []
    total_balance = total_used = total_gained = 0
    for name, u in valid_users:
        ...
        total_used += used
        total_gained += gained
        seen_t = _t(u.get("last_seen"))
        login_t = _t(u.get("last_login"))
        mins = (now - seen_t).total_seconds() / 60 if seen_t else None
        online = mins is not None and mins <= ONLINE_MIN
        user_rows.append({
            "name": name,
            "online": online,
            "status": "🟢 ログイン中" if online else ("⚪ オフライン" if seen_t else "－ 未訪問"),
            "seen_t": seen_t, "login_t": login_t, "created_t": _t(u.get("created_at")),
            "points": pts, "used": used, "gained": gained,
            "vip": _is_premium_until(u.get("premium_until")),
            "login_count": int(u.get("login_count") or 0),
            "visit_count": int(u.get("visit_count") or 0),
            "today": name in today_viewers,
            "login_today": bool(login_t and login_t.strftime("%Y/%m/%d") == today),
        })
    online_n = sum(1 for r in user_rows if r["online"])
    ...
    c3.metric("今日ログインした人", f"{login_today_n}人")
    c4.metric("今日の訪問(延べ)", f"{int(days.get(today, 0))}回")
    d1, d2, d3, d4 = st.columns(4)
    d1.metric("今日訪問した会員", f"{len(today_viewers)}人")
    d2.metric("今日のゲスト訪問(延べ)", f"{int(guest_days.get(today, 0))}回")
    ...
    with tab_users:
        ...
        with f1:
            q = str(st.text_input("ユーザー名で検索", key="adm_q") or "").strip().casefold()
        sorters = {
            "最終訪問が新しい順": (lambda r: r["seen_t"] or datetime.min, True),
            "最終ログインが新しい順": (lambda r: r["login_t"] or datetime.min, True),
            "ポイント残高が多い順": (lambda r: r["points"], True),
            "ポイント残高が少ない順": (lambda r: r["points"], False),
            "累計訪問が多い順": (lambda r: r["visit_count"], True),
            "累計ログインが多い順": (lambda r: r["login_count"], True),
            "登録が新しい順": (lambda r: r["created_t"] or datetime.min, True),
            "ユーザー名順": (lambda r: r["name"].casefold(), False),
        }
        with f2:
            sort_name = st.selectbox("並び順", list(sorters.keys()), key="adm_sort")
        g1, g2, g3 = st.columns(3)
        with g1:
            only_online = st.checkbox("ログイン中のみ", key="adm_online")
        with g2:
            only_today = st.checkbox("今日訪問した人のみ", key="adm_today")
        with g3:
            only_vip = st.checkbox("VIPのみ", key="adm_vip")
        shown = [r for r in user_rows
                 if (not q or q in r["name"].casefold())
                 and (not only_online or r["online"])
                 and (not only_today or r["today"])
                 and (not only_vip or r["vip"])]
        keyf, rev = sorters.get(sort_name, sorters["最終訪問が新しい順"])
        shown.sort(key=keyf, reverse=rev)
        table = [{
            "状態": r["status"], "ユーザー名": r["name"], "ポイント残高": r["points"],
            "会員": "VIP" if r["vip"] else "", "最終ログイン": _fmt(r["login_t"]), "最終訪問": _fmt(r["seen_t"]),
            "今日の訪問": "✓" if r["today"] else "", "累計ログイン": r["login_count"], "累計訪問": r["visit_count"],
            "消費pt": r["used"], "付与pt": r["gained"], "登録日": _fmt(r["created_t"], True),
        } for r in shown]
        st.write(f"{len(shown)} / {len(user_rows)} 人")
        if table:
            st.dataframe(table, hide_index=True)
            csv_buf = io.StringIO()
            csv_writer = csv.DictWriter(csv_buf, fieldnames=list(table[0].keys()))
            csv_writer.writeheader()
            csv_writer.writerows(table)
            st.download_button("この表をCSVで保存", data=csv_buf.getvalue().encode("utf-8-sig"),
                               file_name=f"users_{now.strftime('%Y%m%d_%H%M')}.csv", mime="text/csv", key="adm_csv")
        else:
            st.info("条件に合うユーザーはいません")
        st.caption("累計訪問=一定時間ごとの訪問回数、累計ログイン=ID・パスワードで入った回数です(この機能の追加後から数えます)。")

    with tab_visits:
        rows14 = []
        for k in range(13, -1, -1):
            d = (now - timedelta(days=k)).strftime("%Y/%m/%d")
            v = viewers.get(d) if isinstance(viewers.get(d), list) else []
            rows14.append({"日付": d, "訪問(延べ)": int(days.get(d, 0)), "会員の訪問者数": len(v),
                           "ゲスト訪問(延べ)": int(guest_days.get(d, 0)), "ログイン(延べ)": int(login_days.get(d, 0))})
        try:
            import pandas as pd
            st.bar_chart(pd.DataFrame({"訪問(延べ)": [r["訪問(延べ)"] for r in rows14],
                                       "ログイン(延べ)": [r["ログイン(延べ)"] for r in rows14]},
                                      index=[r["日付"][5:] for r in rows14]))
        except Exception:
            pass
        st.dataframe(list(reversed(rows14)), hide_index=True)
        st.caption("訪問(延べ)は、同じ人でも一定時間あけて来ると増えます。ゲストは個人を区別できないため、延べ回数だけです。")
        st.markdown("#### 今日訪問した会員")
        st.write("、".join(today_viewers) if today_viewers else "まだいません")
        st.caption(f"昨日訪問した会員: {'、'.join(yday_viewers) if yday_viewers else 'なし'}")
        st.markdown("#### 最近ログインした人")
        recent = sorted([r for r in user_rows if r["login_t"]], key=lambda r: r["login_t"], reverse=True)[:30]
        if recent:
            st.dataframe([{"ユーザー名": r["name"], "ログイン日時": _fmt(r["login_t"]), "累計ログイン": r["login_count"],
                           "ポイント残高": r["points"]} for r in recent], hide_index=True)
        else:
            st.write("まだ記録がありません(この機能の追加後のログインから記録されます)")

    with tab_points:
        p1, p2, p3 = st.columns(3)
        p1.metric("残高合計", f"{total_balance}pt")
        p2.metric("付与合計", f"{total_gained}pt")
        p3.metric("消費合計", f"{total_used}pt")
        st.caption(f"今日の消費 {int(used_days.get(today, 0))}pt　今日の付与 {int(gained_days.get(today, 0))}pt　／　昨日の消費 {int(used_days.get(yday, 0))}pt　昨日の付与 {int(gained_days.get(yday, 0))}pt")
        st.markdown("#### 残高ランキング(上位30人)")
        st.dataframe([{"ユーザー名": r["name"], "ポイント残高": r["points"], "消費pt": r["used"], "付与pt": r["gained"]}
                      for r in sorted(user_rows, key=lambda r: r["points"], reverse=True)[:30]], hide_index=True)
        st.markdown("#### ポイント使用履歴")
        st.caption("新しい順。直近80件。")
        log_rows = []
        for row in reversed(point_log[-80:]):
            if isinstance(row, dict):
                log_rows.append({"日時": row.get("time", ""), "ユーザー": row.get("user", ""),
                                 "増減": int(row.get("delta") or 0), "理由": row.get("reason", ""), "残高": row.get("balance", "")})
        if log_rows:
        ...
        if st.button("ポイントを手動で足す"):
            ...
            def _grant(current):
                ...
                return current
            update_json_file(USERS_FILE, {}, _grant, backup=True)
            if not result["ok"]:
    ...
    with tab_diag:
        ...
        st.markdown("#### メモリ使用量")
        _mem = memory_status()
        if _mem["rss_mb"] is None:
            st.caption("この環境ではメモリ使用量を取得できません。")
        else:
            _limit = _mem["limit_mb"]
            m1, m2, m3 = st.columns(3)
            m1.metric("いま使用中", f"{_mem['rss_mb']:.0f} MB")
            m2.metric("起動後の最大", f"{(_mem['peak_mb'] or 0):.0f} MB")
            m3.metric("上限", f"{_limit:.0f} MB" if _limit else "不明")
            if _limit and _mem["rss_mb"] / _limit > 0.8:
                st.warning("メモリの使用量が上限の80%を超えています。上限を超えると、自動で再起動されます。")
        st.caption("全員のアクセスが、1つのプロセスのメモリを共有しています。Renderの Logs には、5分ごとに [mem] として記録されます。")
    ...
    for i in range(len(st.session_state.schars)):
        ...
        with b:
            if i > 0:
                st.button("消す", key=f"scdel_{i}", on_click=_delete_simple_char, args=(i,))
    ...
    if st.session_state.simple_busy:
        st.session_state.simple_busy = False   # 先に下ろす(途中で中断されても、次に開いた時に自動で再課金・再生成されない)
        ...
        if not parts and not chars:
            ...
            try:
                with st.spinner("生成中…"):
                    charged = take_points(spec["cost"], reason="画像生成")
                    used_seed = seed_value if seed_value is not None else secrets.randbelow(4294967296)
                    img = nai_request(
